...
class PinRo extends Component {
  ...
  componentDidMount() {
    ...
    if (
      localStorage.location_pin_ro !== undefined &&
      localStorage.location_pin_ro !== ""
    ) {
      this.setState({ location: localStorage.location_pin_ro });
    }
    if (
      localStorage.any_pin_ro !== undefined &&
      localStorage.any_pin_ro !== ""
    ) {
      this.setState({ any: localStorage.any_pin_ro });
    }
    if (
      localStorage.date_from_pin_ro !== undefined &&
      localStorage.date_from_pin_ro !== null
    ) {
      this.setState({ startDate: localStorage.date_from_pin_ro });
    }
    if (
      localStorage.date_to_pin_ro !== undefined &&
      localStorage.date_to_pin_ro !== null
    ) {
      this.setState({ endDate: localStorage.date_to_pin_ro });
    }
    if (
      localStorage.sort_pin_ro !== undefined &&
      localStorage.sort_pin_ro !== null
    ) {
      this.setState({ sort: localStorage.sort_pin_ro });
    }
    if (
      localStorage.filter_pin_ro !== undefined &&
      localStorage.filter_pin_ro !== null
    ) {
      this.setState({ filter: localStorage.filter_pin_ro });
    }
    if (
      localStorage.status_pin_ro !== undefined &&
      localStorage.status_pin_ro !== null
    ) {
  ...
  handleParameter(pageNumber) {
    ...
    this.props.dispatch(
      FetchPin(pageNumber, this.props.auth.user.id, where, "", "ro")
    );
  }
